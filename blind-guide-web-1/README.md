...
**Your API key is NEVER exposed in the code!**

Instead:
1. Users enter their OWN OpenAI API key in Settings
2. Key is stored in `localStorage` (on their device only)
3. Key goes directly from their browser → OpenAI
4. You (the host) never see their key

This is the safest approach because:
- No server-side code needed
- No proxy to maintain
- Users control their own API usage
- No shared key to be leaked
